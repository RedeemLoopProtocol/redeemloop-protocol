...
v0.6.0 起，Rune recheck 可配合 `createFailoverRuneIndexerAdapter(...)` 使用多 indexer fallback。请求体传入 `manualReviewOnIndexerError: true` 时，如果 indexer lagging 或不可用，API 会返回 `202` 并将 PaymentIntent 推进到 `manual_review`。
