...
For Shopify private-app mark-as-paid alpha support, see [Shopify Mark-as-Paid Adapter Alpha](SHOPIFY_ADAPTER_ALPHA.md).
...
- Indexer recovery: use `createFailoverRuneIndexerAdapter(...)` for primary/fallback indexers. Pass `manualReviewOnIndexerError: true` when the merchant wants indexer lag to move the PaymentIntent to `manual_review`.
...
For trusted EVM settlement, record the broadcast tx hash, then call `settlement/evm/recheck`. The API fetches the receipt through `RPC_URL` or chain-specific `EVM_RPC_URLS`, verifies the ERC-20 `Transfer` log, counts confirmations, and creates a trusted proof.
...
Shopify private-app alpha support uses Admin GraphQL `orderMarkAsPaid` after RedeemLoop settlement confirmation. Configure `SHOPIFY_SHOP_DOMAIN`, `SHOPIFY_ADMIN_ACCESS_TOKEN`, `SHOPIFY_ADMIN_API_VERSION`, and `SHOPIFY_WEBHOOK_SECRET`, then call `GET /v1/diagnostics/shopify` before setting `RELAYER_DRY_RUN=false`.

Do not market this as a Shopify payment app until a reviewed Shopify app path exists.
...
Shopify private-app mark-as-paid alpha 支持请见 [Shopify Mark-as-Paid Adapter Alpha](SHOPIFY_ADAPTER_ALPHA.md)。
...
- Indexer recovery：使用 `createFailoverRuneIndexerAdapter(...)` 配置 primary/fallback indexers。当商户希望 indexer lag 时进入人工处理，可传入 `manualReviewOnIndexerError: true`，让 PaymentIntent 进入 `manual_review`。
...
可信 EVM settlement 流程中，先记录广播 tx hash，再调用 `settlement/evm/recheck`。API 会通过 `RPC_URL` 或按 chainId 配置的 `EVM_RPC_URLS` 读取 receipt，校验 ERC-20 `Transfer` log，计算确认数，并创建 trusted proof。
...
Shopify private-app alpha 支持会在 RedeemLoop settlement confirmation 后调用 Admin GraphQL `orderMarkAsPaid`。配置 `SHOPIFY_SHOP_DOMAIN`、`SHOPIFY_ADMIN_ACCESS_TOKEN`、`SHOPIFY_ADMIN_API_VERSION` 和 `SHOPIFY_WEBHOOK_SECRET` 后，先调用 `GET /v1/diagnostics/shopify`，再把 `RELAYER_DRY_RUN=false`。

在 reviewed Shopify app 路径完成前，不要把它宣传为 Shopify payment app。
